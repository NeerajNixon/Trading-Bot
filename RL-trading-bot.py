...
class CustomEnv:
    ...
    def __init__(self, df, initial_balance=1000, lookback_window_size=50, Render_range = 100):
        ...
        self.Actor = Actor_Model(input_shape=self.state_size, action_space = self.action_space.shape[0], learning_rate=self.lr, optimizer = self.optimizer)
        self.Critic = Critic_Model(input_shape=self.state_size, action_space = self.action_space.shape[0], learning_rate=self.lr, optimizer = self.optimizer)
    ...
    def act(self, state):
        ...
        prediction[0] = prediction[0] + (1-(prediction[0]+prediction[1]+prediction[2]))
